...
//      }
	...
	public void testNoVectorCodegen(CodegenExec exec, boolean print) {
		...
		for (int i = 0; i < repetition; i ++) {
			...
			assert (collect.getResultCount() == rowCount);
		}
		...
		for (int i = 0; i < repetition; i ++) {
			...
			while (input.hasNext()) {
				...
				for (int k = 0; k < row.numRows(); k++) {
					//for (int j = 0; j < 1; j++) {
					value3 = row.getCols(0).getLong(k);
